...
from schema import RequestSchema, ResponseSchema, TokenResponse, UserSigupSchema, UserSiginSchema ,ProductSchema,DeleteProduct
...
@router.post('/test')
async def test(request: RequestSchema, db: Session = Depends(get_db)): #get_db->config.py
    try:
        print(request)
...
@router.post('/signup')
async def signup(request: UserSigupSchema, db: Session = Depends(get_db)): #UserSignupSchema จาก Schema
    try:
        # insert user to db
        # Users จาก Model
...
async def login(request: UserSiginSchema, db: Session = Depends(get_db)):
    try:
       # find user by username
        ...
        if not pwd_context.verify(request.password, _user.password): #pwd_context เข้ารหัส
...
async def delete(request: DeleteProduct, db: Session = Depends(get_db)):
    try:
        # delete User by id
        pdb.set_trace()
        _delete = BaseRepo.delete_by_id(db,DeleteProduct,request.product_id)
